        ...
        <div class="kanaf-card h-100 bg-white shadow-sm">
            <div class="position-relative overflow-hidden">
                <img src="{{ asset('Uploads/orphans/' . $info->personal_photo_path) }}"
                    alt="Default.png" class="card-img-top w-100"
                    style="height: 250px; object-fit: cover;" referrerpolicy="no-referrer">

                <span class="badge position-absolute top-0 right-0 bg-primary-green px-3 py-2 fw-semibold rounded-3 text-white m-3"
                    style="right: 14px; left: auto; z-index: 5;">
                    {{ $info->country }} - {{ $info->city }}
                </span>

                <span class="badge position-absolute top-0 left-0 px-3 py-2 fw-semibold rounded-3 text-white m-3 bg-primary-green"
            ...
            <div class="card-body p-4">
                ...
                <div class="d-flex flex-wrap gap-1 mb-3">
                    <span class="badge bg-danger-subtle text-danger border border-danger-subtle px-2.5 py-1 rounded-pill"
                        style="font-size: 0.75rem; font-weight: 700;">{{ $info->orphan_type }}</span>
                    <span class="badge bg-info-subtle text-dark border border-info-subtle fs-7 px-2.5 py-1 rounded-pill"
                        style="font-size: 0.75rem; font-weight: 700;">{{ $info->urgency_level }}</span>
                </div>

                <div class="d-flex gap-3 text-muted text-small mb-3" style="font-size: 0.85rem; font-weight: 600;">
                    <span><i class="bi bi-calendar3 text-primary-green me-1"></i>العمر: {{ $info->age }} سنوات</span>
                    <span><i class="bi bi-shield-heart text-primary-green me-1"></i>الحالة: {{ $info->health_status }}</span>
                </div>
                ...
                <div class="mb-4">
                    <div class="d-flex justify-content-between text-small mb-1" style="font-size: 0.82rem; font-weight: 700;">
                        <span class="text-muted">درجة اكتمال الكفالة</span>
                        <span class="fw-semibold text-success">0%</span>
                    </div>
                    <div class="progress" style="height: 6px; background-color: var(--light-gray); border-radius: 999px; overflow: hidden;">
                        <div class="progress-bar" role="progressbar" style="width: 0%; background-color: var(--secondary-gold);"
                ...
                <div class="border-top pt-3 d-flex align-items-center justify-content-between">
                    <div>
                        <span class="text-muted d-block text-small mb-1" style="font-size: 0.75rem; font-weight: 700;">مبلغ الكفالة</span>
                        <span class="fs-4 fw-black text-primary-green">$ {{ $info->required_amount }}</span>
                    ...
                    <div class="d-flex gap-2">
                        ...
                        <a href="{{ route('step1', $info->id) }}" class="btn btn-secondary btn-sm px-3 fw-bold rounded-pill">اكفل</a>
                    </div>
                </div>
            </div>
        </div>
...
@empty
    {{-- إذا لم يُرجع الاستعلام أي أيتام، يُترك فارغاً لتقوم الـ JavaScript بإظهار رسالة الـ Empty State --}}
@endforelse
