...
<head>
    ...
    <title>لوحة الإدارة - إدارة المستخدمين</title>
    ...
    <div class="dashboard-wrapper">
        ...
        <div class="sidebar d-flex flex-column" id="kanaf-sidebar-wrapper">
            ...
            <ul class="sidebar-menu flex-grow-1" id="dynamic-menu-list">
                ...
                <li class="menu-item active" id="menu-users">
        ...
        <div class="main-content">
            ...
            <div class="dashboard-header">
                <div class="d-flex align-items-center gap-3">
                    ...
                    <h4 class="fw-bold mb-0 text-dark">مركز إدارة بيانات وعضويات المستخدمين</h4>
                </div>
                <div class="d-flex align-items-center gap-3">
                    <div class="dropdown">
            ...
            <div class="dashboard-container">
                ...
                <!-- عرض رسائل النجاح الناتجة عن تجميد أو تفعيل الحساب -->
                @if (session('success'))
                    <div class="alert alert-success text-right text-small border-0 shadow-sm mb-4" role="alert">
                        ...
                        <div class="kanaf-table-card">
                            ...
                            <div class="kanaf-table-header bg-white">
                                <h5 class="fw-bold text-dark mb-0"><i
                                        class="bi bi-people-fill text-primary-green me-1"></i> الأعضاء
                                    والعضويات المشتركة بالمنظومة</h5>
                                ...
                                <table class="table text-right text-small">
                                    <thead>
                                        <tr>
                                            <th>معرف العضوية</th>
                                            <th>الاسم والصفة</th>
                                            <th>البريد الإلكتروني الحاصل</th>
                                            <th>نوع العضوية الممنوحة</th>
                                            <th>تاريخ تدشين التسجيل</th>
                                            <th>حالة الحساب الكلية</th>
                                    ...
                                    <tbody id="admin-users-tbody">
                                        ...
                                        @forelse($users as $user)
                                            <tr>
                                                <!-- توليد المعرف الرقمي ديناميكياً -->
                                                <td class="font-monospace">USR-{{ $user->id }}</td>
                                                <td>
                                                    <div class="d-flex align-items-center gap-2">
                                                        ...
                                                        <strong
                                                            class="text-dark text-small">{{ $user->name }}</strong>
                                                    </div>
                                                </td>
                                                <td class="font-monospace text-muted">{{ $user->email }}</td>
                                                <td>
                                                    <!-- فحص الأدوار وعرض الشارات بنفس الستايل الأصلي -->
                                                    @if ($user->role === 'admin')
                                                        <span
                                                            class="badge bg-danger px-2.5 py-1 text-small rounded-2">إداري</span>
                                                    @elseif($user->role === 'guardian')
                                                        <span
                                                            class="badge bg-warning text-dark px-2.5 py-1 text-small rounded-2">وصي</span>
                                                    @else
                                                        <span
                                                            class="badge bg-success px-2.5 py-1 text-small rounded-2">متبرع</span>
                                                    @endif
                                                </td>
                                                <!-- جلب تاريخ التسجيل أو تاريخ إنشاء الحساب بالخلفية -->
                                                <td>{{ $user->registration_date ? \Carbon\Carbon::parse($user->registration_date)->format('Y-m-d') : $user->created_at->format('Y-m-d') }}
                                                </td>
                                                <td>
                                                    <!-- فحص وعرض حالة التفعيل -->
                                                    @if (($user->status ?? 'active') === 'active')
                                                        <span class="badge-kanaf badge-active">نشط ومفعل</span>
                                                    @else
                                                        <span class="badge-kanaf badge-stopped">موقوف مؤقتاً</span>
                                                    @endif
                                                </td>
                                                <td>
                                                    <div class="d-flex gap-2 justify-content-center">
                                                        ...
                                                        <form
                                                            ...
                                                            @else
                                                                <button type="submit"
                                                                    class="btn btn-outline-success btn-sm">تنشيط</button>
                                                            @endif
                                                        </form>
                                                    </div>
                                                </td>
                                        ...
                                        @empty
                                            <tr>
                                                <td colspan="7" class="text-center text-muted py-4">لا يوجد
                                                    مستخدمين مسجلين حالياً.</td>
                                            </tr>
                                        @endforelse
