...
<head>
    ...
    <title>لوحة الإدارة - الحسابات المركزية</title>
        ...
        <div class="main-content">
            <div class="dashboard-header">
                <div class="d-flex align-items-center gap-3">
                    ...
                    <h4 class="fw-bold mb-0 text-dark">العمليات والتدقيقات المالية المركزية</h4>
                </div>
                <div class="d-flex align-items-center gap-3">
                    <div class="dropdown">
                        <button class="btn btn-outline-secondary dropdown-toggle d-flex align-items-center gap-2"
                            type="button" id="userMenu" data-bs-toggle="dropdown" aria-expanded="false">
                            <img src="../assets/images/admin.jpg" alt=" " class="rounded-circle"
            ...
            <div class="dashboard-container">
                <!-- عرض تنبيهات النجاح عند الضغط على الأزرار -->
                @if (session('success'))
                ...
                <div class="row g-4 mb-4">
                    <div class="col-md-4">
                        <div class="stats-card">
                            <div class="stats-card-info">
                                <!-- عرض إجمالي الحسابات ديناميكياً -->
                                <h3>$ {{ number_format($total_amount, 0) }}</h3>
                                <p>إجمالي التحصيلات الخيرية المقبوضة</p>
                            </div>
                    ...
                    <div class="col-md-4">
                        <div class="stats-card">
                            <div class="stats-card-info">
                                <h3>100% خيرية</h3>
                                <p>نسبة التشغيل والاستقطاع لليتيم</p>
                            </div>
                    ...
                    <div class="col-md-4">
                        <div class="stats-card">
                            <div class="stats-card-info">
                                <!-- عدد الدفعات الإجمالي ديناميكياً -->
                                <h3>{{ $payments_count }} دفعة مالية</h3>
                                <p>حوالات وتبرعات مرصودة بالمنظومة</p>
                            </div>
                            <div class="stats-card-icon bg-warning-subtle text-warning">
                                <i class="bi bi-journal-check border-warning shadow-xs"></i>
                            </div>
                        </div>
                    </div>
                </div>
                        ...
                        <div class="kanaf-table-card">
                            <div class="kanaf-table-header bg-white">
                                <h5 class="fw-bold text-dark mb-0"><i
                                        class="bi bi-cash-stack text-primary-green me-1"></i> السجل العام والتحقق
                                    المالي لمدفوعات كنف</h5>
                                <button class="btn btn-primary btn-sm"><i class="bi bi-plus-circle me-1"></i> تسجيل
                                    تبرع نقدي مباشر</button>
                            </div>
                            ...
                            <div class="table-responsive">
                                <table class="table text-right text-small">
                                    <thead>
                                        <tr>
                                            <th>معرف المعاملة</th>
                                            <th>الجهة المتبرعة</th>
                                            <th>الطفل المكفول</th>
                                            <th>المبلغ المودع</th>
                                            <th>تاريخ ووسيلة العملية</th>
                                            <th>حالة وتفويض الدفعة</th>
                                    ...
                                    <tbody id="admin-payments-tbody">
                                        @forelse($payments as $payment)
                                            <tr>
                                                <!-- معرف المعاملة: يعرض الـ transaction_id أو ينشئ كود بناءً على رقم الكفالة -->
                                                <td class="font-monospace">
                                                    {{ $payment->transaction_id ?? ($payment->bank_reference_number ?? 'TRX-480' . $payment->id) }}
                                                </td>
                                                <td class="fw-semibold text-dark">
                                                    {{ $payment->sponsor->name ?? 'كافل خير مجهول' }}</td>
                                                <td>اليتيم:
                                                    <strong>{{ $payment->orphan->name ?? $payment->orphan->first_name }}</strong>
                                                </td>
                                                <td class="fw-bold text-success">
                                                    {{ number_format($payment->amount_paid, 0) }} $</td>
                                                <td>
                                                    <span
                                                        class="d-block text-small text-dark">{{ $payment->start_date }}</span>
                                                ...
                                                <td>
                                                    @if ($payment->payment_status == 'paid')
                                                        <span
                                                            class="badge bg-success text-caption px-3 py-1 rounded-pill">مؤكدة</span>
                                                    ...
                                                    @else
                                                        <span
                                                            class="badge bg-danger text-caption px-3 py-1 rounded-pill">فشلت</span>
                                                    @endif
                                                </td>
                                                <td>
                                                    <div class="d-flex gap-2 justify-content-center">
                                                        @if ($payment->payment_status == 'paid')
                                                            <button class="btn btn-outline-secondary btn-sm"
                                                                disabled><i class="bi bi-shield-fill-check"></i> مدققة
                                                                ومقيدة</button>
                                                        @else
                                                            <!-- زر الموافقة والتفويض يدعم الـ Form التابع له -->
                                                            <form
                                                                action="{{ route('approve_payment', $payment->id) }}"
                                                                method="POST" class="d-inline">
                                                                @csrf
                                                                <button type="submit"
                                                                    class="btn btn-success btn-sm"><i
                                                                        class="bi bi-check-lg"></i> موافقة
                                                                    وتفويض</button>
                                                            </form>
                                                        @endif

                                                        <!-- زر الشطب والحذف نهائياً -->
                                                        <form action="{{ route('payments_delete', $payment->id) }}"
                                                            method="POST" class="d-inline"
                                                            onsubmit="return confirm('هل أنت متأكد من شطب هذه المعاملة المالية نهائياً؟')">
                                                            @csrf
                                                            @method('DELETE')
                                                            <button type="submit"
                                        ...
                                        @empty
                                            <tr>
                                                <td colspan="7" class="text-center text-muted py-4">لا توجد عمليات
                                                    تدقيق مالي مسجلة بالنظام حالياً.</td>
                                            </tr>
                                        @endforelse
                                    </tbody>
                                </table>
...
<div class="d-flex justify-content-between align-items-center p-3 border-top bg-white"
    ...
    <div class="text-secondary text-small fw-semibold">
        ...
        <span class="badge px-2 py-1 mx-1"
            style="background-color: #e8f5e9; color: #0f5b38; border: 1px solid #a3d9a5;">{{ $payments->firstItem() ?? 0 }}</span>
        إلى
        <span class="badge px-2 py-1 mx-1"
            style="background-color: #e8f5e9; color: #0f5b38; border: 1px solid #a3d9a5;">{{ $payments->lastItem() ?? 0 }}</span>
        من أصل
        <span class="fw-bold text-dark mx-1">{{ $payments->total() }}</span>
        دفعة مالية
    </div>
