...
<body>
    ...
    <div class="dashboard-wrapper">
        ...
        <div class="main-content">
            ...
            <div class="dashboard-container">
                ...
                <div class="bg-primary-green text-white p-4 p-md-5 rounded-4 shadow-sm mb-4"
                    ...
                    <div class="max-w-xl">
                        ...
                        <p class="mb-0 text-white-50">مرحباً بك مجدداً في لوحة المتابعة الشاملة لليتامى المكفولين تحت
                            اسمك الكريم.
                            هنا يمكنك تتبع التقارير الدراسية وسجلات الدفع بأعلى كفاءة وجدية.</p>
                    </div>
                </div>
                    ...
                    <div class="col-md-4">
                        <div class="stats-card">
                            <div class="stats-card-info">
                                <h3 id="dash-active-count">{{ $sponsorships->count() }}</h3>
                ...
                <!-- Interactive Layout Grid with Chart & Tables -->
                <div class="row g-4">
                    <!-- Monthly donations timeline map -->
                    <div class="col-lg-7">
                        ...
                        <div class="bg-white p-4 rounded-4 shadow-sm border h-100">
                            <div class="d-flex justify-content-between align-items-center mb-4">
                                <h5 class="fw-bold mb-0 text-dark"><i
                                        class="bi bi-people-fill text-primary-green me-2"></i> كفالاتي
                                    السارية</h5>
                                <a href="{{ route('sponsorships') }}"
                                    class="text-caption text-primary-green text-decoration-none">عرض الكل <i
                                        class="bi bi-chevron-left text-small align-middle"></i></a>
                            </div>
                            ...
                            <div class="d-flex flex-column gap-3">
                                @forelse($sponsorships as $sponsorship)
                                            ...
                                            <div class="d-flex align-items-center gap-3">
                                                ...
                                                <div>
                                                    ...
                                                    <span class="text-muted text-small d-block">
                                                        ...
                                                        العمر: {{ $sponsorship->orphan->age }} سنوات
                                                    </span>
                                                </div>
                                            </div>
                                    ...
                                    @endif
                                @empty
                                    <p class="text-muted text-center">لا توجد كفالات نشطة حالياً.</p>
                                @endforelse
    ...
    <script type="module">
        import {
            renderSponsorPaymentChart
        } from '../assets/js/charts.js';

        document.addEventListener("DOMContentLoaded", () => {
            renderSponsorPaymentChart('payment-chart');
            const orphans = getOrphans();
            const sponsorships = getSponsorships();
            const listContainer = document.getElementById('active-sponsorships-list');
            const mySponsorships = sponsorships.filter(s => s.sponsorId === 201 || s.sponsorId === 205);
            document.getElementById('dash-active-count').innerText = mySponsorships.length;
            const myPayments = getPayments();
            const totalSum = myPayments.reduce((acc, p) => acc + p.amount, 0);
            document.getElementById('dash-total-paid').innerText = "$ " + totalSum;
        });
    </script>
</body>
